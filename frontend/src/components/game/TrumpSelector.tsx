...
export function TrumpSelector({ isMyTurn }: TrumpSelectorProps) {
  ...
  const { getSuitAnalysis } = useGameStore();
  const showSuitOverlay = useSettingsStore((state) => state.showSuitOverlay);
        ...
        {SUITS.map(suit => {
          ...
          return (
            <div key={suit} className="relative w-full">
              ...
              {analysis && showSuitOverlay && (
                <div className="absolute -top-14 left-0 right-0 bg-gradient-to-b from-black/95 to-black/30 rounded-lg p-1.5 pointer-events-none shadow-lg z-10">
                  <div className="flex flex-col gap-0.5 text-[9px] font-bold">
                    ...
                    <div className="text-[8px] opacity-90 text-center">
                      <span className="text-green-300">{analysis.visits} sim</span>
                    </div>
                  </div>
                </div>
              )}
