...
/**
 * AI analysis for a suit selection decision
 */
export interface SuitAnalysis {
  ...
  expectedScore: number;  // Expected score change if declaring this suit as trump
  confidence: number;  // 0-1 confidence in the analysis
  visits: number;  // Number of MCTS simulations that explored this suit
  rank: number;  // Rank among all suit options (1 = best)
}
...
export interface AIAnalysisEvent {
  ...
  suits?: SuitAnalysis[];  // For trump selection decisions
